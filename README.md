...
게임 보안 시스템이 `dxgi.dll`을 충돌 프로그램으로 감지하면 게임 Win64 폴더의 `dxgi.dll`을 제거하고, 설치기에서 `winmm.dll` 프록시를 선택해 다시 설치해 보세요. `winmm.dll`도 차단되면 원복하고 사용을 중단하는 것을 권장합니다.
